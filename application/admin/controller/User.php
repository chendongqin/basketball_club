...
class User extends Adminbase{
    ...
    public function ban(){
        $userId = $this->request->param('id',0,'int');
        $banStatus = $this->request->param('ban',1,'int');
        $user = Db::name('user')->where('Id',$userId)->find();
        if(empty($user))
            return $this->returnJson('用户不存在');
        $res = Db::name('user')->update(['Id'=>$userId,'ban'=>$banStatus]);
        if(!$res)
            return $this->returnJson('禁用用户失败!请确认后重试!');
        return $this->returnJson('禁用用户成功!',true,1);
    }
